test_that("ot_indices_1d handles univariate output", {
  dat <- sobol_fun(1000)
  result <- ot_indices_1d(dat[["x"]], dat[["y"]], M = 10)

  expect_s3_class(result, "gsaot_indices")
  expect_named(result, c("method", "indices", "bound", "x", "y",
                         "separation_measures", "partitions", "boot"))
  expect_true(all(paste0("X", 1:8) %in% rownames(result$indices)))
})

test_that("ot_indices_1d handles different p", {
  dat <- ishi_homma_fun(1000)
  result <- ot_indices_1d(dat[["x"]], dat[["y"]], M = 10, p = 5)

  expect_s3_class(result, "gsaot_indices")
  expect_named(result, c("method", "indices", "bound", "x", "y",
                         "separation_measures", "partitions", "boot"))
  expect_true(all(paste0("X", 1:3) %in% names(result$indices)))
})

test_that("ot_indices_1d handles bootstrapping", {
  dat <- sobol_fun(1000)
  result <- ot_indices_1d(dat[["x"]], dat[["y"]], M = 10,
                          boot = TRUE, R = 100)

  expect_s3_class(result, "gsaot_indices")
  expect_named(result, c("method", "indices", "bound", "x", "y",
                         "separation_measures", "partitions", "boot",
                         "indices_ci", "separation_measures_ci",
                         "bound_ci", "R", "type", "conf", "W_boot"))
  expect_true(all(paste0("X", 1:8) %in% rownames(result$indices)))
})
